...
#Define set ooerations mumber/function/symbol
...
if rankMPI == 0: 	#I am master
	...
	NSend=0		#Counter of elements of the list sent
	...
	while PendRec:
		...
		if NSend<ListSize:
			dataMS=[NSend,List[NSend][0],List[NSend][1]]	#data from Master NSend,a,b
			commMPI.send(dataMS,dest=dataSL[2],tag=List[NSend][2])
			NSend+=1
			PendRec+=1


	#All work done -> sent a terminate message
